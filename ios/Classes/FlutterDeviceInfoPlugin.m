...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.constants = [self constantsToExport];
    }
    return self;
}
...
{
  if ([@"getConstants" isEqualToString:call.method]) {
    result(self.constants);
  } else if([@"IPAddress" isEqualToString:call.method]){
      [self getIPAddress:call result:result];
...
-(void)getIPAddress:(FlutterMethodCall*)call result:(FlutterResult)result
{
...
-(void)batteryLevel:(FlutterMethodCall*)call result:(FlutterResult)result
{
